...
export default function DnsAndDomainsReference(): JSX.Element {
    ...
    <>
      ...
      <DocsArticle
        ...
        nextStep={{
          ...
          description:
            "AI site builder, chat conversations, and per-user provider BYOK.",
        }}
      >
        <p>
          Domain-related functionality lives in three routers because
          they have different authorisation boundaries.{" "}
          <code>domainSearch.*</code> and parts of <code>domains.*</code>{" "}
          are public (anyone can check availability).{" "}
          <code>dns.*</code> is admin-only because zone and record
          management carries blast-radius beyond any single user.
        </p>

        <h2>domainSearch.* — public availability checks</h2>

        <h3><code>domainSearch.search</code></h3>
        <p>
  ...
  includeAiSuggestions: z.boolean().optional(),
        ...
        <p>
          Returns only the <em>available</em> names, taken/unknown
          counts, optional AI-suggested alternatives, and optional
          trademark warnings (medium + high risk only). Cached —{" "}
          <code>result.cached</code> tells you whether the response
          was served from the cache.
        </p>

        <h2>domains.* — register + manage</h2>

        <h3><code>domains.search</code></h3>
        <p>
          Public <em>query</em>. Same shape as{" "}
          <code>domainSearch.search</code> but against the underlying
          OpenSRS lookup. Returns an <code>AvailabilityResult[]</code>{" "}
          for every requested TLD (including ones that failed). Use{" "}
          <code>domainSearch.search</code> if you want the AI +
          trademark layer on top.
        </p>

        <h3><code>domains.getPricing</code></h3>
        <p>
          Public <em>query</em>. Input{" "}
          <code>{`{ domain: string, years?: number }`}</code>. Returns
          wholesale + retail price (in micro-dollars), markup percent,
          and currency. Years defaults to 1.
        </p>

        <h3><code>domains.register</code></h3>
        <p>
          Admin <em>mutation</em>. Registers a domain on behalf of a
          user. Input carries the domain, contact details, and the{" "}
          <code>userId</code> the domain will be assigned to. Admin-
          only because the request charges the platform's OpenSRS
          account — a user-facing "buy this domain" flow is a separate
          piece of work that wraps this procedure with an escrow step.
        </p>

        <h3><code>domains.renew</code></h3>
        <p>
          Admin <em>mutation</em>. Input{" "}
          <code>{`{ domain: string, years: number }`}</code>. Extends
          the registration via OpenSRS.
        </p>

        <h3><code>domains.listMyDomains</code></h3>
        <p>
          Protected <em>query</em>. No input. Returns every domain
          registered to the current user, including expiry dates and
          nameserver state.
        </p>
        ...
        <Callout tone="warn" title="Admin-only router">
          Every procedure on <code>dns.*</code> is admin-gated. A
          rogue user cannot enumerate zones, flip nameservers, or
          create records — attempting to call these without the admin
          role returns <code>UNAUTHORIZED</code>.
        </Callout>
          ...
          items={[
            {
              term: "dns.listZones",
              description:
                "Admin query. No input. Returns every zone managed by the platform.",
            },
            {
              term: "dns.getZone",
              description:
            ...
            },
            {
              term: "dns.updateZone",
              description:
                "Admin mutation. Patches zone-level settings (TTL defaults, DNSSEC).",
            },
            {
              term: "dns.deleteZone",
              description:
          ...
          items={[
            {
              ...
              description:
                "Admin query. Input { zoneId }. Returns every record in the zone.",
            },
            {
              term: "dns.getRecord",
              description:
                "Admin query. Input { id }. Returns a single record row.",
            },
            {
            ...
            },
            {
              ...
              description:
                "Admin mutation. Input { id, ...patch }. Partial update on any record.",
            },
            {
              term: "dns.deleteRecord",
              description:
                "Admin mutation. Input { id }. Removes the record upstream and locally.",
            },
            {
              term: "dns.bulkImport",
              description:
...
const registered = await trpc.domains.register.mutate({
  ...
  /* contact details... */
        ...
        <Callout tone="note">
          <strong>What's not here yet:</strong> a user-facing "buy this
          domain" procedure that wraps <code>domains.register</code>{" "}
          with Stripe escrow. That ships as part of the billing go-live
          — until then, registration is an ops action.
        </Callout>
