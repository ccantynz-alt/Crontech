...
import { CreateUserInput, PaginationInput } from "@back-to-the-future/schemas";
...
export const usersRouter = router({
  ...
  update: adminProcedure.input(UpdateUserInput).mutation(async ({ ctx, input }) => {
    ...
    const result = await ctx.db.update(users).set(updateData).where(eq(users.id, id)).returning();
  ...
  delete: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const result = await ctx.db.delete(users).where(eq(users.id, input.id)).returning();
