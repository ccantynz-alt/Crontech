...
const FAQ_ITEMS: FAQItem[] = [
  {
    ...
    answer:
      "Yes. Client-side GPU inference via WebGPU is completely free on all plans. Models like Llama 3.1 8B run at 41 tokens/second directly in your browser. You pay nothing for these tokens -- your device does the work.",
  },
  {
    question: "What happens when I hit the free tier limits?",
    answer:
      "Your existing projects continue to work. You simply cannot create new AI generations beyond the daily limit. Upgrade to Pro anytime to unlock unlimited generations and access to edge and cloud compute tiers.",
  },
  {
    question: "How does the three-tier compute model work?",
    answer:
      "Workloads automatically route to the optimal tier: your browser GPU for small models (free), Cloudflare edge network for mid-range tasks (sub-50ms latency), or H100 cloud GPUs for heavy inference and training. The platform handles routing transparently.",
  },
  {
    question: "Can I switch plans at any time?",
    answer:
      "Yes. Upgrade or downgrade instantly. When upgrading, you get immediate access to all new features. When downgrading, your current billing period completes before the change takes effect. No lock-in contracts.",
  },
  {
    question: "Do you offer discounts for startups or open-source projects?",
    answer:
      "Yes. We offer 50% off Pro for verified startups (under $5M ARR) and free Pro access for qualifying open-source projects. Contact our sales team with details about your project.",
  },
];
  ...
  return (
    <div
      class={`relative flex flex-col rounded-2xl p-6 transition-all duration-300 ${
        props.plan.highlighted ? "shadow-2xl" : ""
      }`}
      style={{
        background: props.plan.highlighted ? "var(--color-bg-elevated)" : "var(--color-bg-subtle)",
    ...
    >
      ...
      <div class="mb-6">
        ...
        >
          <div class="flex items-baseline gap-1">
            <span class="text-4xl font-bold tracking-tight" style={{ color: "var(--color-text)" }}>
              ${price()}
            </span>
      ...
      <button
        ...
        class={`mb-6 w-full rounded-xl py-3 text-sm font-semibold transition-all duration-200 ${
          props.plan.highlighted ? "hover:brightness-110" : ""
        }`}
        style={{
          background: props.plan.highlighted ? "var(--color-primary)" : "var(--color-bg-muted)",
          color: props.plan.highlighted
            ? "var(--color-primary-text)"
            : "var(--color-text-secondary)",
          border: props.plan.highlighted ? "none" : "1px solid var(--color-border)",
        }}
      >
        {props.plan.ctaLabel}
      </button>
        ...
        <For each={props.plan.features}>
          {(feature) => (
            <div class="flex items-start gap-2.5">
              <span class="mt-0.5 text-sm" style={{ color: props.plan.accentColor }}>
                &#10003;
              </span>
...
function FAQSection(props: { item: FAQItem }): JSX.Element {
  ...
  return (
    <div style={{ "border-bottom": "1px solid var(--color-border)" }}>
      ...
      >
        <span class="pr-8 text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>
          {props.item.question}
        </span>
        <span
          class={`shrink-0 text-lg transition-transform duration-300 ${isOpen() ? "rotate-45" : ""}`}
    ...
    <div class="min-h-screen" style={{ background: "var(--color-bg)" }}>
      ...
      <div class="relative overflow-hidden">
        <div class="relative mx-auto max-w-6xl px-6 pt-16 pb-12 text-center">
          ...
          <p class="mx-auto mt-4 max-w-2xl text-base" style={{ color: "var(--color-text-muted)" }}>
            Free to start. On-device AI inference at $0. Edge and cloud compute when you need more
            power. No hidden fees, no surprise bills, no credit card to begin.
          </p>
      ...
      <div class="mx-auto max-w-4xl px-6 pb-10">
        ...
        >
          ...
          <p class="mt-2 text-sm" style={{ color: "var(--color-text-secondary)" }}>
            The first wave of paid customers lock in{" "}
            <span class="font-semibold" style={{ color: "var(--color-text)" }}>
              50% off any plan for life
            </span>{" "}
            and a direct line to the team building the platform.
          </p>
        </div>
      </div>
      ...
      <div class="mx-auto max-w-6xl px-6 pb-16">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          <For each={PLANS}>{(plan) => <PlanCard plan={plan} isAnnual={isAnnual()} />}</For>
        </div>
      </div>

      {/* Feature Comparison Table */}
      <div class="mx-auto max-w-5xl px-6 pb-20">
        <h2 class="mb-8 text-center text-2xl font-bold" style={{ color: "var(--color-text)" }}>
          Feature Comparison
        </h2>
        <div
          ...
          <table class="w-full">
            <thead>
              <tr style={{ "border-bottom": "1px solid var(--color-border-strong)" }}>
                ...
                <th
                  class="px-4 py-4 text-center text-xs font-semibold uppercase tracking-widest"
                  style={{ color: "var(--color-text-muted)" }}
                >
                  Free
                </th>
      ...
      <div class="mx-auto max-w-3xl px-6 pb-20">
        ...
        >
          <For each={FAQ_ITEMS}>{(item) => <FAQSection item={item} />}</For>
        </div>
      </div>
        ...
        >
          <div class="relative">
            <h3 class="text-2xl font-bold" style={{ color: "var(--color-text)" }}>
              Ready to build on the next decade's stack?
            </h3>
            <p class="mt-2 text-sm" style={{ color: "var(--color-text-secondary)" }}>
