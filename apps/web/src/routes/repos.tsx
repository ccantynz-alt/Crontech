    ...
    >
      ...
      <div class="flex items-start justify-between gap-3">
        <div class="flex items-center gap-2.5 min-w-0">
          <img src={props.repo.owner.avatar_url} alt="" class="h-6 w-6 rounded-lg" loading="lazy" />
          <div class="flex min-w-0 flex-col">
            <div class="flex items-center gap-2">
              ...
              <Show when={props.repo.fork}>
                <span class="text-[9px]" style={{ color: "var(--color-text-faint)" }}>
                  fork
                </span>
              </Show>
  ...
  return (
    ...
    >
      ...
      <div class="flex min-w-0 flex-1 flex-col gap-0.5">
        <span class="truncate text-xs font-medium" style={{ color: "var(--color-text)" }}>
          {message()}
        </span>
        <span class="text-[10px]" style={{ color: "var(--color-text-faint)" }}>
...
function IssueRow(props: { issue: Issue }): JSX.Element {
  return (
    ...
    >
        ...
        style={{
          background:
            props.issue.state === "open"
              ...
              : "color-mix(in oklab, var(--color-text-muted) 12%, transparent)",
      ...
      <span class="shrink-0 text-[10px]" style={{ color: "var(--color-text-faint)" }}>
        {timeAgo(props.issue.updated_at)}
      </span>
    </a>
  );
}
...
function BranchRow(props: { branch: Branch; isDefault: boolean }): JSX.Element {
  return (
    ...
    >
      ...
      <span class="flex-1 text-xs font-medium font-mono" style={{ color: "var(--color-text)" }}>
        {props.branch.name}
      </span>
      ...
      >
        ...
        <Show when={repos().length > 0}>
          ...
          >
            ...
            >
              {repoStats().public} public
            </span>
            <span
              class="rounded-full px-2 py-0.5 text-[10px] font-medium"
              style={{ background: "var(--color-bg-muted)", color: "var(--color-text-muted)" }}
            >
              {repoStats().private} private
            </span>
            <span
              class="rounded-full px-2 py-0.5 text-[10px] font-medium"
              style={{ background: "var(--color-bg-muted)", color: "var(--color-text-muted)" }}
            >
              {repoStats().languages} languages
            </span>
          </div>
        </Show>
            ...
            >
              <Show
                ...
                fallback={
                  ...
                  >
                    <span class="text-xs" style={{ color: "var(--color-danger)" }}>
                      {error()}
                    </span>
                  </div>
                }
                ...
                <Show
                  ...
                  fallback={
                    <EmptyState
                      ...
                      subtitle={search() ? "Try a different search" : "No repositories available"}
                    />
                  }
                >
          ...
          fallback={
            <div class="flex flex-1 flex-col items-center justify-center gap-4">
              ...
              <p
                ...
                style={{ color: "var(--color-text-secondary)" }}
              >
                Choose a repo from the list to view commits, pull requests, issues, branches, and CI
                status.
              </p>
            </div>
          }
        >
          {(repo) => (
            <>
              ...
              {/* Detail Tabs */}
              <div
                class="flex items-center gap-1 px-6 py-2"
                style={{
                  "border-bottom": "1px solid var(--color-border)",
                  background: "var(--color-bg-elevated)",
                }}
              ...
              <div class="flex-1 overflow-y-auto px-6 py-4">
                ...
                >
                    ...
                    <Show
                      ...
                      fallback={
                        <EmptyState
                          icon="&#128221;"
                          title="No commits"
                          subtitle="This repository has no commits yet"
                        />
                      }
                    >
                      <div class="flex flex-col">
                        <For each={commits()}>{(commit) => <CommitRow commit={commit} />}</For>
                      </div>
                    </Show>
                  </Show>
                    ...
                    <Show
                      ...
                      fallback={
                        <EmptyState
                          ...
                          subtitle="No pull requests found"
                        />
                      }
                    >
                      <div class="flex flex-col">
                    ...
                    <Show
                      ...
                      fallback={
                        <EmptyState
                          ...
                          subtitle="All clear — no open issues"
                        />
                      }
                    >
                      <div class="flex flex-col">
                    ...
                    <Show
                      ...
                      fallback={
                        <EmptyState
                          icon="&#9740;"
                          title="No branches"
                          subtitle="This repository has no branches"
                        />
                      }
                    >
                      <div class="flex flex-col">
                  ...
                  <Show when={activeTab() === "ci"}>
                    <Show
                      when={ciRuns().length > 0}
                      fallback={
                        <EmptyState
                          icon="&#9889;"
                          title="No CI runs"
                          subtitle="No workflow runs found for this repository"
                        />
                      }
                    >
                      <div class="flex flex-col">
                        <For each={ciRuns()}>{(run) => <CIRunRow run={run} />}</For>
                      </div>
                    </Show>
                  </Show>
                </Show>
