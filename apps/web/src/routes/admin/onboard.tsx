...
const CRONTECH_ENV_VARS: EnvVar[] = [
  {
    ...
    description: "Edge SQLite primary URL",
    required: true,
    service: "Database",
  },
  ...
  {
    key: "GLUECRON_WEBHOOK_SECRET",
    description: "Secret for Gluecron → Crontech push hook",
    required: false,
    service: "Git",
  },
  {
    key: "GLUECRON_GIT_BASE_URL",
    description: "Gluecron git base URL (replaces GitHub origin)",
    required: false,
    service: "Git",
  },
  {
    key: "DEPLOY_AGENT_SECRET",
    description: "Secret for the internal deploy agent",
    required: true,
    service: "Deploy",
  },
  {
    key: "ALECRAE_API_KEY",
    description: "AlecRae transactional email API key",
    required: false,
    service: "Email",
  },
  {
    key: "ORCHESTRATOR_URL",
    description: "Tenant deploy orchestrator URL",
    required: false,
    service: "Deploy",
  },
];
...
interface AnalysisResult {
  found: Array<{
    ...
    value: string;
    croontechKey?: string;
    status: "mapped" | "unknown" | "sensitive";
  }>;
  missing: EnvVar[];
  aiNotes: string;
  cleanupItems: string[];
...
function OnboardContent(): JSX.Element {
  ...
  const isSensitive = (key: string): boolean => /secret|password|token|key|pwd|pass/i.test(key);
  ...
  const analyse = (): void => {
    ...
    const found = Object.entries(parsed).map(([key, value]) => ({
      ...
      status: (mappings.get(key) || croontechKeys.has(key) ? "mapped" : "unknown") as
        | "mapped"
        | "unknown"
        | "sensitive",
    }));
    ...
    const missing = CRONTECH_ENV_VARS.filter((v) => v.required && !coveredCroontechKeys.has(v.key));
  ...
  const generateEnvFile = (): void => {
    ...
    const lines = [
      ...
      ...a.found.filter((f) => f.croontechKey).map((f) => `${f.croontechKey}=  # was: ${f.key}`),
      "",
      "# ── Required — add values ──",
      ...
      <div class="mx-auto max-w-4xl px-6 py-8">
        ...
        <div class="mb-8">
          ...
          <p class="mt-1 text-sm" style={{ color: "var(--color-text-faint)" }}>
            Migrate any project onto Crontech. Paste your existing config — we'll detect what maps
            across, what's missing, and what to clean up.
          </p>
        </div>
        ...
        <div class="mb-8 flex items-center gap-6">
          ...
          <StepBadge
            ...
            label="Analyse"
            active={step() === "analyse"}
            done={["review", "export"].includes(step())}
          />
          <div class="h-px flex-1" style={{ background: "var(--color-border)" }} />
        ...
        <Show when={step() === "paste"}>
          ...
          >
            ...
            <p class="mb-5 text-sm" style={{ color: "var(--color-text-muted)" }}>
              ...
              file contents below. Values are analysed locally — nothing is sent to a server.
            </p>
                ...
                >
                  ...
                  <div class="flex flex-col gap-2">
                    <For each={a().missing}>
                      {(v) => (
                        <div
                          class="flex items-start justify-between rounded-lg px-3 py-2"
