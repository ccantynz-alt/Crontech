...
import { friendlyError, useMutation, useQuery } from "../lib/use-trpc";
  ...
  const plans = useQuery(
    () =>
      trpc.billing.getPlans
        .query()
        .catch(() => [] as Awaited<ReturnType<typeof trpc.billing.getPlans.query>>),
    { key: "plans" },
  );

  const checkout = useMutation((input: { priceId: string }) =>
    trpc.billing.createCheckoutSession.mutate(input),
  );
  const portal = useMutation((input: { customerId: string }) =>
    trpc.billing.createPortalSession.mutate(input),
  );
      ...
      <div class="mx-auto max-w-6xl px-6 py-8">
        ...
        <div class="mb-8">
          ...
          <p class="mt-1 text-sm" style={{ color: "var(--color-text-faint)" }}>
            ...
            >
              Your plan, billed through Stripe. Invoices, cards, tax details, and cancellation all
              live in the Stripe portal linked below.
            </Show>
          </p>
        </div>
        ...
        >
          <Show when={error()}>
            {(msg) => (
              <div
                class="mb-6 rounded-xl px-4 py-3 text-xs font-medium"
                style={{
                        ...
                        <ul class="mb-5 flex flex-col gap-2">
                          <For each={features()}>
                            {(feature) => (
                              ...
                              >
                                <span class="mt-0.5" style={{ color: "var(--color-success)" }}>
                                  &#10003;
                                </span>
                                <span>{feature}</span>
                              </li>
          ...
          >
            <h2 class="text-lg font-semibold" style={{ color: "var(--color-text)" }}>
              Everything else lives in Stripe
            </h2>
            <p class="mt-2 text-sm" style={{ color: "var(--color-text-secondary)" }}>
              Invoices, payment methods, billing address, tax IDs, and cancellation all live inside
              Stripe's billing portal. One click from here, real state, real receipts, real PCI
              compliance — no duplicate forms, no drifted data.
            </p>
            ...
            >
              Usage-based charts (API calls, AI tokens, storage, seats) arrive with the
              usage-metering block (BLK-011). Until that pipeline is live, this page shows only
              things we can verify against Stripe.
            </p>
          </div>
        </Show>
      </div>
